"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PrestamistaService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const prestamista_entity_1 = require("./entities/prestamista.entity");
let PrestamistaService = class PrestamistaService {
    constructor(prestamistaRepository) {
        this.prestamistaRepository = prestamistaRepository;
    }
    create(createPrestamistaInput) {
        const prestamista = this.prestamistaRepository.create(createPrestamistaInput);
        if (!prestamista.estado) {
            prestamista.estado = 'activo';
        }
        return this.prestamistaRepository.save(prestamista);
    }
    findAll(estado = 'todos') {
        const whereCondition = estado === 'todos' ? {} : { estado };
        return this.prestamistaRepository.find({ where: whereCondition });
    }
    findOne(id) {
        return this.prestamistaRepository.findOne({ where: { idprestamista: id } });
    }
    async update(id, updatePrestamistaInput) {
        const prestamista = await this.prestamistaRepository.preload({
            idprestamista: id,
            ...updatePrestamistaInput,
        });
        if (!prestamista) {
            throw new common_1.NotFoundException(`Prestamista with ID ${id} not found`);
        }
        return this.prestamistaRepository.save(prestamista);
    }
    async remove(id) {
        const prestamista = await this.prestamistaRepository.findOneBy({ idprestamista: id });
        if (prestamista) {
            prestamista.estado = 'inactivo';
            return this.prestamistaRepository.save(prestamista);
        }
        return null;
    }
};
exports.PrestamistaService = PrestamistaService;
exports.PrestamistaService = PrestamistaService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(prestamista_entity_1.Prestamista)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], PrestamistaService);
//# sourceMappingURL=prestamista.service.js.map